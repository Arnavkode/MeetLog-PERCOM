...
SCAN_CHECKPOINTS = [5, 15, 35, 55, 75, 95, 115] 
...
def build_suggestion(attentive_percent, avg_temp, avg_humidity, avg_light, avg_co2, door_status, motion_status, llm_tip):
    base_suggestion = (
        ...
        f"💡 Suggestions for next meeting:\n{llm_tip}"
    )
...
def check_global_checkpoints():
    ...
    for cp in SCAN_CHECKPOINTS:
        if min_scans >= cp and cp not in fired_checkpoints:
            ...
            # Collect full owner summary, like meeting end (but without LLM)
...
def _build_owner_summary(interval="meeting_end"):
    try:
        total_att, total_scans = 0, 0
        rows = []
        for uid, u in users_data.items():
            percent = (u["attentive_count"]/u["total_scans"]*100) if u["total_scans"] else 0
            rows.append({"user_id": uid, "attentive_percent": round(percent,1), "scans": u["total_scans"]})
            total_att += u["attentive_count"]
            total_scans += u["total_scans"]

        overall = (total_att/total_scans*100) if total_scans else 0

        summary_file = f"{MEETING_SUMMARY_DIR}/owner_{interval}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        pd.DataFrame(rows + [{"user_id":"ALL","attentive_percent":round(overall,1),"scans":total_scans}]) \
            .to_csv(summary_file, index=False)

        logging.info(f"✅ [{interval}] Owner summary saved at {summary_file}")
    except Exception as e:
        logging.error(f"❌ Failed to build owner summary: {e}")
...
def predict():
    ...
    # Here 12 classes (adjust if your model outputs different length)
...
def start_meeting():
    ...
    meeting_state["ended_users"].discard(user_id)  # in case user re-joined
...
def log_attention():
    ...
    # logging.info(f"Attention Request Payload Size : {len(str(data))} bytes")
    ...
    # logging.info(f"Attention Response Payload Size : {len(str(resp))} bytes")
...
def attention_poke():
    try:
        ...
        # logging.info(f"Attention Poke Request Payload Size : {len(str(data))} bytes")
        ...
        # logging.info(f"Attention Poke Response Payload Size : {len(str(resp))} bytes")
...
def end_meeting():
    try:
        ...
        prompt = f"""
        You are a meeting attentiveness assistant. You analyze:
        - Attention percentage (0–100%)
        - Observed behavior tokens (from 12 predefined activities)
        - Environmental factors (temperature, humidity, light, CO₂)
        - Room status (door open/closed, motion detected)

        Attentive activities: 
        1. Presenting while sitting
        2. Head-nodding while sitting
        3. Typing on a laptop while sitting
        4. Taking notes while sitting
        5. Writing on a board while standing
        6. Erasing the board while standing
        7. Presenting while standing

        Disengaged activities:
        8. Drinking water from a bottle while standing
        9. Sipping coffee from a cup while standing
        10. Scrolling a phone while sitting
        11. Sipping coffee from a cup while sitting
        12. Drinking water from a bottle while sitting

        Your task:
        1. Provide 1–2 short, actionable suggestions to sustain or improve focus:
        - Attention level guidance:
            • Low (<50%) → suggest shifting away from disengaged actions and recommend multiple active attentive activities to re-engage.  
            • Moderate (50–85%) → suggest 1–2 reinforcement strategies that add more active participation.  
            • High (>85%) → suggest sustaining behaviors that keep focus without overload.  
        - Behavior guidance:
            • If disengaged behaviors are detected → recommend switching to one of the 7 attentive activities.  
            • If attentive behaviors are detected → reinforce continuation and suggest a light improvement.  
        - Environmental guidance:
            • If comfort ranges are exceeded (Temp >28°C, Humidity >70%, CO₂ >800 ppm, Light <150 lux) → suggest a corrective action (ventilate, adjust lighting, cool room, etc.).  
        2. Provide a concise engagement summary (1–2 sentences).  
        3. Keep responses very short (2–3 lines max), clear, and user-friendly. Avoid long explanations or multiple bullet points.
        ...
        logging.info(
            f"LLM Prompt tokens: {len(prompt.split())}, Response tokens: {len(llm_tip.split()) if llm_tip else 0}"
        )
        ...
        productivity_score = compute_meeting_productivity(
            [attentive_percent],  # per-user attentiveness
            {
                "light": avg_light,
                "temp": avg_temp,
        ...
        # === only now reset this user's live counters (safe; snapshot kept) ✅ ===
        users_data[user_id] = {
            "last_status": "unknown",
            "attentive_count": 0,
            "total_scans": 0,
...
def _collect_owner_summary_from_snapshot(snapshot_dict):
    ...
    # === Load latest environment window (e.g. last 300 rows) ===
...
def _collect_owner_summary_data():
    ...
    Returns:
    {
      "overall_attentive_percent": float,
      "productive": bool,                 # based on MPS >= 60
      "productivity": {A,E,MPS,env_scores},
      "user_summaries": [{user_id, attentive_count, total_scans, attentive_percent}],
      "total_attentive": int,
      "total_scans": int
    }
    """
...
def _save_owner_summary_files(summary_data):
    ...
    Returns paths to summary_file, bar chart, pie chart, combined chart.
    """
    ...
    logging.info(f"✅ Saved meeting summary CSV: {summary_file}")
    ...
    logging.info(f"🧪 Plotting users={user_ids}, percents={attentive_percents}")


    # === Bar chart (Per-user Attentive %) ===
    ...
    # === Pie chart (Overall Attention Split) ===
...
,
    ...
    plt.title("Overall Attention Split")
    plt.tight_layout()
    plt.savefig(pie_path, dpi=150)
    plt.close()
    logging.info(f"✅ Saved pie chart: {pie_path}")
    ...
    logging.info(f"✅ Saved combined summary: {combined_chart_path}")
    logging.info(f"📊 Meeting Productivity Score (MPS): {summary_data['productivity']['MPS']:.1f}%")
...
if __name__ == "__main__":
    # Start background resource monitor thread first
    
    # threading.Thread(target=log_resource_usage, args=(10,), daemon=True).start()
